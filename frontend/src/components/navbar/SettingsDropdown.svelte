<script lang="ts">
    import { logoutUser } from "@api/auth-api";
    import { tick } from "svelte";
    import { checkAuthStatus, isAdmin } from "@stores/auth-store";

    let isFocused = $state(false);
    let dropdownElement: HTMLDivElement | null = $state(null); // Ref to the main dropdown div

    checkAuthStatus();

    async function handleLogout() {
        const success = await logoutUser();
        if (success) {
            window.location.href = "/iniciar-sessao/";
        } else {
            const { showAlert, AlertType, AlertPosition } = await import(
                "@components/alert/alert"
            );
            showAlert(
                "Erro ao terminar sessão.",
                AlertType.ERROR,
                AlertPosition.TOP,
            );
        }
    }

    function handleFocus() {
        isFocused = true;
    }

    async function handleBlur(event: FocusEvent) {
        // Use tick to allow the relatedTarget to be updated correctly
        await tick();
        // Check if the new focused element is inside the dropdown content
        if (!dropdownElement?.contains(event.relatedTarget as Node)) {
            isFocused = false;
        }
    }

    // SVG path data for the settings icon
    const settingsIconPath =
        "m14 4.3117c0-.72443-.5873-1.3117-1.3117-1.3117h-1.3766c-.7244 0-1.3117.58727-1.3117 1.3117v.25706c0 .42782-.28712.79949-.68224.96353-.39526.16411-.85548.10157-1.1581-.20106l-.27666-.27665c-.58242-.58243-1.52672-.58243-2.10915 0l-.71928.71928c-.58242.58242-.58242 1.52672 0 2.10914l.27666.27666c.30263.30263.36517.76284.20106 1.1581-.16404.39512-.53571.68224-.96352.68224h-.25707c-.72443 0-1.3117.5873-1.3117 1.3117v1.3766c0 .7244.58727 1.3117 1.3117 1.3117h.40807c.33822 0 .63289.2259.75572.5411.11803.3028.06177.6495-.16807.8794l-.31838.3184c-.54624.5462-.54624 1.4318 0 1.9781l.96484.9648c.48299.483 1.26607.483 1.74906 0l.04451-.0445c.37602-.376.95241-.4395 1.43926-.2256.46838.2058.81329.6492.81329 1.1608v.1158c0 .7244.5873 1.3117 1.3117 1.3117h1.3766c.7244 0 1.3117-.5873 1.3117-1.3117v-.2571c0-.4278.2871-.7995.6822-.9635.3953-.1641.8555-.1016 1.1581.2011l.0871.0871c.4421.4421 1.1589.4421 1.601 0l1.2275-1.2275c.4421-.4421.4421-1.1589 0-1.601l-.0871-.0871c-.3027-.3026-.3652-.7628-.2011-1.1581.164-.3951.5357-.6822.9635-.6822h.2571c.7244 0 1.3117-.5873 1.3117-1.3117v-1.3766c0-.7244-.5873-1.3117-1.3117-1.3117h-.1158c-.5116 0-.955-.34491-1.1608-.81329-.2139-.48685-.1504-1.06324.2256-1.43926l.0445-.0445c.483-.48299.483-1.26606 0-1.74904l-.9649-.96487c-.5462-.54623-1.4318-.54623-1.978 0l-.3184.31838c-.2299.22984-.5766.2861-.8794.16807-.3152-.12283-.5411-.4175-.5411-.75573zm-2 10.6883c1.6569 0 3-1.3431 3-3s-1.3431-3-3-3-3 1.3431-3 3 1.3431 3 3 3z";
</script>

<div class="dropdown dropdown-end" bind:this={dropdownElement}>
    <div
        tabindex="0"
        role="button"
        class="btn btn-ghost m-1"
        onfocus={handleFocus}
        onblur={handleBlur}
    >
        <svg
            fill="none"
            viewBox="0 0 24 24"
            class="w-6 h-6 stroke-current settings-icon {isFocused
                ? 'rotate'
                : ''}"
            xmlns="http://www.w3.org/2000/svg"
        >
            <path
                clip-rule="evenodd"
                stroke="currentColor"
                d={settingsIconPath}
                fill-rule="evenodd"
                stroke-linejoin="round"
                stroke-width="1.5"
            ></path>
        </svg>
    </div>
    <ul
        tabindex="0"
        class="dropdown-content menu bg-base-200 rounded-box z-[100] w-52 p-2 shadow-lg border border-base-content/10"
        onblur={handleBlur}
    >
        {#if $isAdmin}
            <li><a href="/admin/">Painel de Administração</a></li>
        {/if}
        <li><a href="/settings/">Definições da Conta</a></li>
        <li>
            <button onclick={handleLogout}>Terminar Sessão</button>
        </li>
    </ul>
</div>

<style>
    .settings-icon {
        transition: transform 0.3s;
    }
    .rotate {
        transform: rotate(90deg);
    }
</style>
