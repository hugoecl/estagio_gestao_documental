<script lang="ts">
  ...
  let {
    ...
    recordId: string;
    showFiles?: boolean;
    files?: Record<string, { name: string; path: string; uploadedAt: string }>;
    onSubmit: (
  ...
  let confirmationAction = $state<"DELETE_RECORD" | "DELETE_FILE" | null>(null);
  ...
  async function handleSubmit(e: SubmitEvent) {
    ...
    const success = await onSubmit(formValues, newFiles);
    ...
    if (success) {
      ...
      showAlert("Erro ao guardar dados", AlertType.ERROR, AlertPosition.TOP);
    }
    isSubmitting = false;
  }
  ...
  async function handleDeleteRecord() {
    ...
    if (success) {
      ...
      showAlert("Erro ao eliminar registo", AlertType.ERROR, AlertPosition.TOP);
    }
  }
    ...
    <form onsubmit={handleSubmit} class="space-y-4">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        {#each fields as field}
          <fieldset class="fieldset" class:md:col-span-2={field.colSpan === 2}>
  ...
  <div class="modal-box">
    ...
    <p class="py-4">
      ...
      {:else if confirmationAction === "DELETE_RECORD"}
        ...
        eliminar este registo? Esta ação não pode ser desfeita e todos os ficheiros
